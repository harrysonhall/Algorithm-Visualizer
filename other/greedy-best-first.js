...
export async function GreedyBestFirstAlgorithm(eventCallerType) {
		...
		while(current !== goal_node_img.parentElement) {
			...
			for(let neighbor of neighbors) {
				...
				if(neighbor[1] !== null && priorityQueue.contains(neighbor[1]) === false && neighbor[1].classList.length === 0) {
					...
					priorityQueue.enqueueByAdjacencyTieBreaker([neighbor[1], fcost, hcost, gcost], checkerBoardTieBreker, neighbor[0]);
